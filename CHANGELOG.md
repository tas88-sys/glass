...
- **Question detection is recall-maximal** (live-tested 2026-05-31) — `isLikelyQuestion` is a NON-question *screen*, not a question *detector*: it drops only empty turns and pure acknowledgement/backchannel and triggers on everything else, letting the model's PASSIVE reply suppress non-questions (panel holds the last answer). A keyword/opener list always left gaps — imperative prompts ("Design a cache", "Compare X and Y"), conjunction-led clauses ("okay so how…"), statement-form asks ("your biggest weakness"), and punctuation-less STT that buries the question mid-utterance — so the heuristic was inverted to guarantee a real question is never dropped.
