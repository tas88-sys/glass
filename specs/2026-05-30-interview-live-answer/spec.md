...
- **FR-002**: Before any LLM call, the system MUST apply a cheap, **balanced** pre-filter (`isLikelyQuestion`) that triggers on a real question SIGNAL but skips the interviewer's declarative monologue, so a low daily LLM quota isn't drained on non-questions (which would otherwise risk failing real questions later). A `them:` turn triggers when ANY of: (1) a `?` appears anywhere; (2) an imperative/interview cue appears (design/compare/explain/give me/walk through/tell me/…); (3) an embedded/indirect question appears ("…the question is what…"); (4) a wh-word or yes/no auxiliary LEADS any clause (leading discourse markers like "okay so" are peeled first). Cue-less statement-form prompts ("your biggest weakness") are the known by-design gap. *(Recall-oriented but quota-aware; PASSIVE remains the final filter for any false trigger. Tuned 2026-05-31 after live STT testing + quota review.)*
