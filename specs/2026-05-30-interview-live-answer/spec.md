...
- **FR-002**: Before any LLM call, the system MUST apply a cheap, **recall-maximal** pre-filter (`isLikelyQuestion`). It is deliberately a NON-question *screen*, not a question *detector*: it drops ONLY empty turns and pure acknowledgement/backchannel ("okay", "got it", "mm-hmm", "sounds good", …); **every other `them:` turn triggers**, and the model's PASSIVE reply is the real filter (it suppresses non-questions — the panel holds the last answer). This guarantees a real question is NEVER dropped, because a keyword/opener list always leaves gaps (imperative prompts "Design a cache", conjunction-led clauses "okay so how…", statement-form asks, punctuation-less STT). *(Recall over precision; PASSIVE is the backstop. AS-3's "non-question → hold last answer" is now enforced by PASSIVE, not the heuristic. Redesigned 2026-05-31 after live STT testing.)*
