...
summary:
  ...
  decisions:
    ...
    - {id: D6, decision: "SummaryView.js ALSO moved OUT of the CLOSED set for an insights-pane LAYOUT fix (FR-019/FR-020, branch feat/live-answer-readability) so the Live Answer and Summary lanes each scroll INDEPENDENTLY within a window-bounded flex region. SummaryView.js gains ONLY scroll/height CSS (:host flex-bounded column; .insights-container flex:1/min-height:0/overflow-y:auto with NO fixed max-height) + a read-only getContentHeight() getter; ListenView.js gains the .insights-pane flex distributor (height:100vh container) and measures both lanes; LiveAnswerView.js keeps its ≤280px scroll and pins via flex:0 0 auto. This NARROWS D5's 'SummaryView.js REMAINS closed' note — only the scroll-layout CSS + getter are added; the summary DATA path is untouched", rationale: "[2026-06-01] Live testing showed the Summary lane was clipped at the window edge during a recording (content after 'Actions' unreachable) and only scrolled after Stop. Root cause: the 700px-capped window + two fixed max-height scrollers (Live Answer 280 + Summary 600) that summed past the cap inside a collapsed height:100% container, with no flex bounding — so the Summary overflowed the OS window with no scrollbar. The fix is a layout restructure that REQUIRED editing SummaryView.js; the summary-update channel, structuredData shape, makeOutlineAndRequests / triggerAnalysisIfNeeded, and the in-memory invariant (C3/C8/FR-011) are unchanged"}
...
### Post-implementation correction #2 — Session 2026-06-01 (Summary-lane clipping / independent lane scrolling)

Further live testing surfaced a **second, distinct** clipping bug — this time in the **Summary** lane, not the Live Answer lane FR-016 addressed. During an active recording the Summary content after the "Actions" heading was **unreachable with no scrollbar**, and a scrollbar appeared only after the user pressed **Stop**. FR-016 resizes the window as the *answer* streams, but the window is capped at 700px, so a resize alone cannot help once the lanes' combined height exceeds the cap:

- **Q: Why was the Summary clipped during recording but scrollable after Stop?** → **A:** `ListenView.adjustWindowHeight` caps the window at `Math.min(700, idealHeight)`, but the two insights lanes were INDEPENDENT scrollers with fixed `max-height` thresholds (Live Answer `280px`, Summary `600px`) inside an auto-height (`height:100%`, i.e. collapsed against the auto host chain) container. Their summed height (`32 + 280 + 600`) exceeded the 700px cap, so the bottom of the Summary overflowed the OS **window edge** with no scrollbar — the Summary's own `overflow-y:auto` only engaged once *its own* content passed `600px`, which happened only after Stop appended the `Follow-Ups` section. Transcript mode never hit this (one lane, `32 + 600 < 700`).
- **Q: What is the fix, and does it keep the lanes independent?** → **A: Yes — the lanes still scroll independently (FR-019).** The fix bounds each lane to a flex layout that fits inside the window instead of relying on fixed `max-height` thresholds that can sum past the cap: the assistant container gets a DEFINITE height (`height:100vh`, not `height:100%`); a non-scrolling `.insights-pane` flex distributor hands the two lanes a definite height to divide; the Live Answer lane stays compact/pinned (`flex:0 0 auto`, its own `≤280px` scroll) and the Summary lane takes the rest (`flex:1 1 0; min-height:0`, its own scroll, NO fixed `max-height`). `adjustWindowHeight` sizes the window to the full content of both lanes up to the cap via `SummaryView.getContentHeight()` (FR-020).
- **Q: Does this re-open the closed summary lane?** → **A: No (data path).** Only `SummaryView.js`'s scroll/height **CSS** plus a read-only `getContentHeight()` getter are added (D6); `ListenView.js` gains the `.insights-pane`. The `summary-update` channel, the `structuredData` shape, `makeOutlineAndRequests` / `triggerAnalysisIfNeeded`, and the in-memory invariant (C3/C8/FR-011) are unchanged. `promptTemplates.js`, `askService.js`, `featureBridge.js`, `listenService.js`, and `preload.js` REMAIN closed.
...
12. **Given** an active recording where the Live Answer history and the Summary together exceed the 700px window cap, **When** the Summary grows past the visible area (e.g. content after the "Actions" heading), **Then** the Summary lane shows its OWN scrollbar and scrolls to the bottom WITHOUT requiring Stop first, and the Live Answer lane scrolls independently above it — neither lane is clipped by the window edge (FR-019/FR-020).
...
#### Improvement 4 — Independent lane scrolling (insights-pane layout) *(post-implementation; branch `feat/live-answer-readability`)*

*(FR-017/FR-018 are reserved as parent-spec references throughout this doc; this spec's next free numbers are FR-019+.)* Live testing after FR-016 surfaced a **second, distinct** clipping bug in the **Summary** lane (see "Post-implementation correction #2"): during a recording the Summary content below the "Actions" heading was unreachable with no scrollbar, becoming scrollable only after Stop. FR-016 resizes the window as the answer streams, but the window is capped at 700px and the two insights lanes were independent scrollers with fixed `max-height`s (280 + 600) that summed past that cap inside a collapsed `height:100%` container — so the Summary overflowed the OS window edge instead of scrolling. These requirements bound each lane to the flex layout while keeping them independently scrollable.

- **FR-019** *(independent, window-bounded lane scrolling)*: In insights mode the Live Answer and Summary lanes MUST each scroll **independently** within a region bounded by the available window height, so neither can overflow the 700px-capped listen window and clip with no scrollbar. The lanes MUST sit in a flex column with a **definite** height (the assistant container is `height:100vh`, NOT `height:100%`, which collapses against the auto-height host chain) under a **non-scrolling** `.insights-pane` flex distributor, so each lane's scroll region is bounded by the flex layout — NOT by a fixed `max-height` that can sum past the window cap. The Live Answer lane MUST be a compact, pinned region (`flex:0 0 auto`) that scrolls within its own `≤280px` `max-height`; the Summary lane MUST take the remaining space (`flex:1 1 0; min-height:0`) and scroll within it, and MUST NOT carry a fixed `max-height`. (Transcript mode is unchanged — it remains a single `stt-view` scroller.)
- **FR-020** *(window sizing reflects the full content of both lanes)*: `ListenView.adjustWindowHeight` MUST size the listen window to show the full content of both lanes up to the 700px cap, after which the lanes scroll. Because the Summary host is now flex-bounded (so its host `scrollHeight` reports only the clipped, visible height — under-measuring and preventing growth), `SummaryView` MUST expose its FULL content height via a read-only `getContentHeight()` (the inner `.insights-container.scrollHeight`) for the measurement; the Live Answer lane's capped host `scrollHeight` already reflects its `≤280px` contribution. The growth signal is the existing `live-answer-updated` event PLUS a mirrored `summary-updated` event (`SummaryView.updated()` → `ListenView` → `adjustWindowHeightThrottled`, added in commit `a0cb5ae`); no IPC channel is added and nothing is persisted.

#### Independence & non-regression (inherited)

- **FR-010**: The changes MUST be purely additive to the summary lane's **data path**. `promptTemplates.js`, `makeOutlineAndRequests`, `triggerAnalysisIfNeeded`, the `summary-update` channel, the `structuredData` payload shape, `askService.js`, `featureBridge.js`, `listenService.js`, and `preload.js` MUST NOT be modified (the parent spec's FR-017 / CLOSED set continues to hold for these). `LiveAnswerView.js` and `ListenView.js` ARE edited for the rendering-correctness fixes FR-013–FR-016 **and** the insights-pane layout fix FR-019/FR-020; `SummaryView.js` IS edited for FR-019/FR-020 — but ONLY its scroll/height CSS (`:host` flex-bounded column; `.insights-container` `flex:1`/`min-height:0`/`overflow-y:auto`, no fixed `max-height`) plus a read-only `getContentHeight()` getter (D6). Every edit preserves the `live-answer-update` / `summary-update` payload shapes and each lane's observable content. If Improvements 1–4 were reverted, the summary lane MUST behave exactly as it does today.
...
Modified (rendering-correctness FR-013–FR-016 + insights-pane layout FR-019/FR-020):
...
- <live-answer-view> :host: flex:0 0 auto (compact/pinned); .live-answer-container keeps its ≤280px scroll (FR-019)
- <summary-view> :host + .insights-container: flex-bounded column scrolling within its allotted space (NO fixed max-height); new read-only getContentHeight() for window sizing (FR-019/FR-020)
- ListenView: assistant container height:100vh; non-scrolling .insights-pane flex distributor; listens for `live-answer-updated` + `summary-updated`; adjustWindowHeight measures both lanes (FR-016/FR-019/FR-020)

New components: NONE (no NEW renderer/UI components; the existing ones are bug-fixed / re-laid-out in place — no new view-mode, toggle, or layout component)
```
...
- **Insights-pane layout edits (FR-019/FR-020)**: `src/ui/listen/ListenView.js` (assistant container `height:100vh`; a non-scrolling `.insights-pane` flex distributor wrapping the two lanes; `adjustWindowHeight` measures Live Answer's capped host `scrollHeight` + Summary's `getContentHeight()`); `src/ui/listen/summary/SummaryView.js` (`:host` flex-bounded column + `.insights-container` `flex:1`/`min-height:0`/`overflow-y:auto` with NO fixed `max-height`; new read-only `getContentHeight()`); `src/ui/listen/summary/LiveAnswerView.js` (`:host` `flex:0 0 auto`; `.live-answer-container` keeps its `≤280px` `max-height` scroll). The summary-lane data path, `summary-update` channel, and `structuredData` shape are unchanged (D6).
...
- **Closed/untouched**: `promptTemplates.js`, `makeOutlineAndRequests`, `triggerAnalysisIfNeeded`, the `summary-update` channel + `structuredData` shape, `askService.js`, `featureBridge.js`, `listenService.js`, `preload.js` (the IPC plumbing from the parent spec needs no change). *`SummaryView.js` is no longer fully closed — only its scroll/height CSS + `getContentHeight()` are touched (D6/FR-019/FR-020); its summary-rendering logic and `summary-update` subscription are unchanged.*
...
- **Improvement 4 (FR-019/FR-020 — insights-pane independent-lane scrolling)** added 2026-06-01: bounded each insights lane to a window-fitting flex layout so the Summary lane scrolls in place during recording (no Stop required) and the Live Answer lane scrolls independently above it. Builds on the mirrored summary resize signal `summary-updated` (commit `a0cb5ae`). This `spec.md` and `CHANGELOG.md` are updated to match; **`acceptance-tests.yaml` SC/MST coverage for FR-019/FR-020 is still pending** (no Electron-renderer harness — these are manual smoke checks like the FR-013–FR-016 MSTs).
